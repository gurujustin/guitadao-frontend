...
function Claim() {
  ...
  const pguitaBalance = useSelector(state => {
    return state.account.balances && state.account.balances.pguita;
  });
  console.log("debug --> Claim / pguitaBalance : ", pguitaBalance);
  const setMax = () => {
    setQuantity(pguitaBalance);
  };
  ...
  const onChangeClaim = async action => {
    ...
    if (action === "claim" && gweiValue.gt(ethers.utils.parseUnits(pguitaBalance, "gwei"))) {
      return dispatch(error("You cannot claim more than your pGUITA balance."));
    }
    await dispatch(changeClaim({ address, action, value: quantity.toString(), provider, networkID: chainID }));
  };
  const hasAllowance = useCallback(
    token => {
      if (token === "pguita") return claimAllowance > 0;
      return 0;
    },
    [claimAllowance],
  );
            ...
            <Grid container spacing={1} alignItems="center" justifyContent="center">
                ...
                !hasAllowance("pguita") ? (
                  <Box className="help-text">
                    <Typography variant="body1" className="stake-note" color="textSecondary">
                      <>
                        First time use <b>pGUITA</b>?
                        <br />
                        Please approve GuitaDao to use your <b>pGUITA</b> for claim GUITA.
                      </>
                    </Typography>
                  </Box>
                ) : (
              ...
              {isAllowanceDataLoading ? (
                ...
                <Grid item xs={12} sm={4} md={4} lg={4}>
                  ...
                  >
                    {txnButtonText(pendingTransactions, "claim", "Claim GUITA")}
                  </Button>
                </Grid>
              ) : (
