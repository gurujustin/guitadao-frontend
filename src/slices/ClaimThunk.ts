...
import { abi as pGUITA } from "../abi/pGUITA.json";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const pguitaContract = new ethers.Contract(addresses[networkID].PGUITA_ADDRESS as string, pGUITA, signer);
    let approveTx;
    let claimAllowance = await pguitaContract.allowance(address, addresses[networkID].PRESALE_ADDRESS);
    ...
    try {
      if (token === "pguita") {
        ...
        approveTx = await pguitaContract.approve(
          addresses[networkID].PRESALE_ADDRESS,
          ethers.utils.parseUnits("1000000000", "gwei").toString(),
        );
      }

      const text = "Approve pGUITA";
    ...
    claimAllowance = await pguitaContract.allowance(address, addresses[networkID].PRESALE_ADDRESS);
